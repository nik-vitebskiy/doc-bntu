...
При первом запуске откроется одноразовая страница создания администратора. Готовых логинов и паролей в новой базе нет. После входа Excel загружается через страницу реестра. Договоры, дополнительные соглашения и заявки поступают из АИС; их сканы и исходные файлы сотрудники прикрепляют к карточкам документов. Файлы хранятся непосредственно в PostgreSQL вместе с остальными данными; каталог `uploads/` используется только миграцией старых установок.
