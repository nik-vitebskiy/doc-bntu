...
from app.models import AnnualDemand, AppUser, Contract, ContractFaculty, Order, OrderItem, Organization, SessionLocal
from app.services.audit_service import AuditActor, AuditBatch
...
def main():
    ...
    try:
        ...
        agreement = register_additional_agreement(
            session, contract, "ТЕСТ-ДС-2026/01", date(2026, 9, 16), admin.id, audit_actor=actor
        )
        with AuditBatch(session, actor, comment="Настроен пример изменений заказа д.с."):
