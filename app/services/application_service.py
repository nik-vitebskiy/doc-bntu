...
from .status_service import APPLICATION_STATUSES
...
def update_application(session, application, number, signed_date, status):
...
def attach_application_scan(session, application, stored_name):
    session.add(Document(organization_id=application.organization_id, application_id=application.id,
