...
def make_template(path="templates/dop_soglashenie.docx", overwrite=False):
    path = Path(path)
    ...
    for value, bold in [("ДОПОЛНИТЕЛЬНОЕ СОГЛАШЕНИЕ", True), ("к договору № {{ contract_number }} от {{ contract_date }}", True), ("г. Минск", False), ("{{ bntu.full_name }} в лице {{ bntu.signer_position }} {{ bntu.signer_name }}, действующего на основании доверенности №{{ bntu.power_of_attorney_number }} от {{ bntu.power_of_attorney_date }}, и {{ org_name }}, адрес: {{ org_address }}, заключили настоящее дополнительное соглашение.", False), ("Приложение: заказ на подготовку кадров ({{ faculty }} факультет).", True)]:
    ...
    doc.add_paragraph("Реквизиты университета", style="Heading 2")
    for value in (
        "{{ bntu.full_name }}",
        "Юридический адрес: {{ bntu.legal_address }}",
        "УНП {{ bntu.unp }}, ОКПО {{ bntu.okpo }}",
        "р/с {{ bntu.bank_account }}",
        "{{ bntu.bank_name }}, БИК {{ bntu.bic }}",
        "{{ bntu.signer_position }} ____________________ {{ bntu.signer_name }}",
    ):
        paragraph = doc.add_paragraph(value)
        paragraph.paragraph_format.space_after = Pt(3)
    doc.add_paragraph("Организация-заказчик ____________________")
    doc.save(path)
